...
class Session(asyncore.dispatcher_with_send):
    ...
    def handle_read(self):
        ...
        if not self.out_buffer:
            self.disconnect()
            return
        if self.out_buffer[0] == 60 and self.__client == None:
        ...
        if (sizeByte < 1 or sizeByte > 3):
            return
        packetLen = buffer.readByte() if sizeByte == 1 else buffer.readShort() if sizeByte == 2 else ((buffer.readByte() & 0xFF) << 16) | ((buffer.readByte() & 0xFF) << 8) | (buffer.readByte() & 0xFF) if sizeByte == 3 else 0
        fullPacketLen = packetLen + sizeByte + 2
        if (packetLen == 0 or length < fullPacketLen):
            return
        self.packetId = buffer.readByte()
